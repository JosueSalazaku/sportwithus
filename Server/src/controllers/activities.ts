...
export const updateActivity = async (req: Request, res: Response) => {  // Corrected function name
  ...
  try {
    ...
    const updatedActivity = await db
      ...
      .where(eq(activities.id, parseInt(id, 10)))  // Ensure the ID is parsed as an integer
      .returning({
        id: activities.id,
        name: activities.name,
        description: activities.description,
...
export const deleteActivity = async (req: Request, res: Response) => {
  ...
  try {
    const deletedActivity = await db
      .delete(activities)
      .where(eq(activities.id, parseInt(activityId, 10)))  // Ensure the ID is parsed as an integer
      .returning({
        id: activities.id,
        name: activities.name,
        description: activities.description,
